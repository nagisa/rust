...
impl<'a,'tcx> Builder<'a,'tcx> {
    ...
    pub fn diverge_cleanup(&mut self) -> Option<BasicBlock> {
        ...
        // * Drops with higher index in the drops array are executed after free drop;
        //
        // NB: The execution order and translation order are reverse here. i.e. free drop is built
        // last, but will be executed first for the given scope. The first block we build is the
        // one with `Resume` terminator, but this block is executed last in any given chain of
        // cleanups.
        {
