...
fn generate_filter_fn<'a, 'tcx>(fcx: &FunctionContext<'a, 'tcx>,
    ...
    //          %arg = call i8* llvm.localrecover(@__rust_try, %parentfp, 0)
    ...
    // `__rust_try` function, which is then in turn passed to the `localrecover`
    // intrinsic (pairing with the `localescape` intrinsic mentioned above).
    // Putting all this together means that we now have a handle to the
    // arguments passed into the `try` function, allowing writing to the stack
    // over there.
    //
